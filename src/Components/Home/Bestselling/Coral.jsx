...
export default function Coral() {
  ...
  return (
    <section className="w-full min-h-[70vh] bg-gradient-to-tr from-sky-400 to-blue-600 py-10">
      {/* Header */}
      ...
      <div className="relative w-full mt-6">
        ...
        >
          {infiniteCoralData.map((item, index) => (
            <div
              key={index}
              className="flex-none w-56 sm:w-64 md:w-72 h-80 rounded-xl bg-white/10 shadow-lg backdrop-blur-sm transform transition hover:scale-105 relative"
            >
              {/* Image */}
              ...
              {/* Add to Cart */}
              <button
                className="absolute bottom-3 left-1/2 -translate-x-1/2 w-[90%] h-10 bg-black text-white font-semibold rounded-md opacity-0 hover:bg-blue-700 transition group-hover:opacity-100"
                onClick={() =>
                  addToCart({
                    id: item.id,
                    title: item.title,
